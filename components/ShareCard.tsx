...
type ShareCardProps = {
  ...
  action48h: FortyEightHourAction;
  minimumCombo: MinimumComboAdvice;
...
export function ShareCard({
  ...
  return (
    <div className="rounded-lg border border-[rgba(226,27,45,0.35)] bg-[linear-gradient(135deg,rgba(226,27,45,0.16),rgba(23,24,32,0.98)_38%,rgba(8,9,13,1))] p-5 sm:p-6">
      ...
      <p className="mt-4 whitespace-pre-line border-t border-[rgba(255,255,255,0.10)] pt-4 text-sm leading-6 text-[#FFFFFF]">
        不可替代性表达句：{positionSentence.text}
      </p>
      <p className="mt-3 text-sm leading-6 text-[#FFFFFF]">
        48小时行动：{action48h.title}
      </p>
      <p className="mt-3 text-sm leading-6 text-[#FFFFFF]">
        先做的公开作品/动作：{minimumCombo.work}
      </p>
